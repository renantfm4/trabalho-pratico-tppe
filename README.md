...
3. [Configuração do Ambiente](#1-configuração-do-ambiente)
4. [Estrutura do Projeto](#2-estrutura-do-projeto)
5. [Como Executar os Testes](#3-como-executar-os-testes)
6. [Critérios de Implementação](#4-critérios-de-implementação-deduplicação-dinâmica)
...
3. Edite a variável **`Path`** e adicione uma nova linha no topo:
   - `%JAVA_HOME%\bin`
4. Reinicie o terminal e verifique com o comando:
   ```bash
   java -version
   ```
...
Para garantir o funcionamento correto e a validação do estágio **Green** do TDD, execute os comandos abaixo no terminal, a partir da raiz do projeto.
...
**Gerar o Relatório de Cobertura de Código (JaCoCo):**
...
Após o término, abra o arquivo gerado em `target/site/jacoco/index.html` no navegador para visualizar os percentuais de cobertura.

### Casos cobertos pelos testes

| Caso | Descrição |
|:----:|:----------|
| 1 | Diferenças tipográficas e de acentuação. |
| 2 | Sobrenome + iniciais simples. |
| 3 | Partículas de ligação ("de", "da", "do") e pontos opcionais. |
| 4 | Iniciais agrupadas (ex.: "VC Junior"). |
| 5 | Unificação de registros pelo menor ID encontrado. |
| Exceção | Validação de entradas nulas, vazias ou com IDs negativos. |
...
A string com a maior "pontuação de integridade" é eleita automaticamente como o **Padrão-Ouro** para todos os registros unificados.